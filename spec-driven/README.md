...
npx @atmmdev/spec-driven

# ou criando numa pasta nova
npx @atmmdev/spec-driven meu-projeto

# não-interativo: escolha os clientes pela flag (ao menos um é obrigatório)
npx @atmmdev/spec-driven --agent=claude,cursor   # Claude + Cursor
```
...
```bash
npx @atmmdev/spec-driven [diretório-alvo] [opções]
```

### Exemplos

```bash
npx @atmmdev/spec-driven                # scaffolda no diretório atual
npx @atmmdev/spec-driven meu-projeto    # cria/usa a pasta ./meu-projeto
npx @atmmdev/spec-driven . --force      # sobrescreve arquivos existentes
npx @atmmdev/spec-driven update         # atualiza só a esteira, preserva seus docs
```
...
> `npx @atmmdev/spec-driven update --agent=codex` materializa o Codex e o mantém atualizado).
